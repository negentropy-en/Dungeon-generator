...
class Room: # Room class representing a rectangular room in the dungeon
    def __init__(self, x, y, width, height, room_id):  # Initializing the room with position, size, and ID; also calculates center point
        self.x = x
        self.y = y
        self.width = width
        self.height = height
...
class Door: # Door class representing a door connecting two rooms; room1 first connected room, room2 second connected room
...
class DungeonGenerator: # Main dungeon generator class; generates rooms, corridors, doors, triangulation, and grid. Dungeon is represented as a grid of walkable (1) and non-walkable (0) cells.
    def __init__(self, width=100, height=100, max_rooms=15, 
                 min_room_size=5, max_room_size=15):
        if width < 10 or height < 10:
            raise ValueError("Dungeon dimensions must be at least 10x10")
        if max_rooms < 1:
            raise ValueError("Must generate at least 1 room")
    ...
    def generate_rooms(self): # Generate non-overlapping rooms within dungeon bounds
        ...
        logger.info(f"Generated {generated} rooms after {attempts} attempts")
        return generated
    ...
    def generate_delaunay(self): # Generate Delaunay triangulation of room centers
        ...
        try:
            ...
            logger.info(f"Created Delaunay triangulation with {len(self.triangulation.triangles)} triangles")
            return True
        except Exception as e:
            logger.error(f"Failed to create Delaunay triangulation: {e}")
            self.triangulation = None
            return False

    def generate_mst(self): # Generate Minimum Spanning Tree (MST) from Delaunay triangulation to connect rooms
        ...
        # The Point objects in triangle.vertices are the SAME objects
        # (same memory addresses) as room.center, so dictionary lookup works
        # correctly without any rounding needed.
        ...
        potential_extra_edges = [
            (self.rooms[u], self.rooms[v]) for u, v, _ in sorted_edges 
            if (self.rooms[u], self.rooms[v]) not in mst_edges and 
               (self.rooms[v], self.rooms[u]) not in mst_edges
        ]
        ...
        logger.info(f"Created MST with {len(mst_edges)} base edges and {len(final_edges)} total edges")
        return True

    def create_corridor_between_rooms(self, room1, room2): # Create an L-shaped corridor between two rooms
    ...
    def generate_grid(self): # Generating 2D grid representation of the dungeon
        try:
            ...
            # Mark corridor cells, value 1 = walkable, same as rooms for pathfinding
            ...
            logger.info(f"Generated {self.width}x{self.height} grid")
            return True
    ...
    @staticmethod
    def get_line_points(x1, y1, x2, y2): # Bresenham's line algorithm to get points between two coordinates
        points = []
    ...
    def generate(self): # Generating complete dungeon with rooms, corridors, doors, triangulation, and grid
        ...
        if num_rooms >= 3:
            ...
            logger.warning(f"Only {num_rooms} rooms generated, skipping MST")
        
        # Generate grid
        if not self.generate_grid():
            raise RuntimeError("Failed to generate grid")
        
        logger.info(f"Dungeon generation complete: {num_rooms} rooms, "
                   f"{len(self.corridors)} corridors, {len(self.doors)} doors")
