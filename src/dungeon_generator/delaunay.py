...
class Triangle:   # Represents a triangle defined by three points    
    ...
    def calculate_circumcircle(self):   # Calculate circumcircle (center and radius) of the triangle
        point_a, point_b, point_c = self.vertices
        determinant = 2 * (point_a.x * (point_b.y - point_c.y) + 
        ...
        ux = ((point_a.x**2 + point_a.y**2) * (point_b.y - point_c.y) +
              (point_b.x**2 + point_b.y**2) * (point_c.y - point_a.y) +
              (point_c.x**2 + point_c.y**2) * (point_a.y - point_b.y)) / determinant
        uy = ((point_a.x**2 + point_a.y**2) * (point_c.x - point_b.x) +
              (point_b.x**2 + point_b.y**2) * (point_a.x - point_c.x) +
              (point_c.x**2 + point_c.y**2) * (point_b.x - point_a.x)) / determinant
        
        radius = math.hypot(point_a.x - ux, point_a.y - uy)
        return (ux, uy), radius

    def in_circumcircle(self, point):   # Check if a point is inside the triangle's circumcircle
        center, radius = self.circumcircle
        if center[0] is None:
            return False
        
        dist = math.hypot(point.x - center[0], point.y - center[1])
        # Using strict inequality with epsilon for Delaunay property; points on the circle are considered outside
        return dist < radius - 1e-10


class DelaunayTriangulation:   # Delaunay triangulation using Bowyer-Watson algorithm    
    ...
    def create_super_triangle(self):   # Create a super-triangle that encompasses all points; super-triangle is large enough to contain all points
    ...
    def triangulate(self): # Performing the Bowyer-Watson triangulation algorithm
        ...
        self.triangles = [t for t in self.triangles 
                         if not any(v in st_vertices for v in t.vertices)]