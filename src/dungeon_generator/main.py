...
def generate_and_visualize(seed):       # Generating and visualizing the dungeon
    if seed is not None:
        random.seed(seed)
    ...
    rooms, corridors, _, triangulation, _ = generator.generate()
    if rooms:
        print(f"Generated {len(rooms)} rooms.")         # Shows triangulation and corridors in the visualization
        visualize_dungeon(
            rooms,
            corridors,
            triangulation,
...
def main():
    ...
    parser.add_argument(
        "--game",
        action="store_true",
        help="Run the interactive Pygame viewer"
    )
    parser.add_argument(
        "--generate",
        action="store_true",
...
if __name__ == "__main__":
    ...
    """Currently works to a good degree, need to figure out why triangulation paths go over the rooms when running the project with --generate"""